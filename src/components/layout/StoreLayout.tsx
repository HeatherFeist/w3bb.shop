  ...
  return (
    <div className="flex min-h-svh flex-col">
      <header className="sticky top-0 z-30 border-b bg-card/95 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 py-3 sm:px-6">
          ...
          <nav className="hidden items-center gap-6 text-sm font-medium sm:flex">
            ...
            {CATEGORIES.map((cat) => (
              <Link key={cat} to={`/shop?category=${cat}`} className="text-muted-foreground hover:text-foreground">
      ...
      <footer className="border-t bg-secondary/40 py-6 text-center text-sm text-muted-foreground">
        ...
        <Link to="/shop" className="mt-1 inline-block underline-offset-2 hover:underline sm:hidden">
          Shop the collection
        </Link>
